...
# # # plaintext = input("Enter the value of the plain text ")
...
# # secretKey = input("")  # 256-bit random encryption key
...
# # print(len(b'\x8b\\,`Kc\xedq\xe7\xd0\xe8^\x10B\x8d\x0f\xdf\xcd\xa1e\xc8T\xcf\x1cg\x1a\xb7\xc6N?\tY'))
...
class AES:
    ...
    def process_block(self, block, inverse=False):
        ...
        for i in range(4):
            self.state.append(
                [block[i], block[i + 4], block[i + 8], block[i + 12]])
    ...
    def shift_rows(self, inverse=False):
        for row in range(1, 4):
            self.state[row] = AES.rotate(
                self.state[row], row if not inverse else -row)
